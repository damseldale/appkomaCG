...
const colorFor = (value, fallback) => /^#[0-9a-f]{6}$/i.test(value || '') ? value : fallback;
...
const drawCharacter = (ctx, object, asset) => {
  ...
  const skin = colorFor(parts.head, '#f2c9a5');
  const hair = colorFor(parts.hair, '#3f2d20');
  const body = colorFor(parts.body, '#4b5563');
  const eye = colorFor(parts.eyes, '#202228');
  const mouth = colorFor(parts.mouth, '#b34b5d');
  const wave = pose === 'wave'; const stride = pose === 'walk' || pose === 'run';
  ctx.lineCap = 'round';
  ctx.fillStyle = skin; ctx.beginPath(); ctx.arc(0, -50, 18, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = hair; ctx.beginPath(); ctx.arc(0, -58, 18, Math.PI, 0); ctx.fill();
  ctx.fillStyle = eye; ctx.beginPath(); ctx.arc(-6, -52, 2, 0, Math.PI * 2); ctx.arc(6, -52, 2, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = mouth; ctx.lineWidth = 2; ctx.beginPath(); if (expression === 'happy') { ctx.arc(0, -48, 6, 0, Math.PI); } else if (expression === 'sad') { ctx.arc(0, -43, 6, Math.PI, Math.PI * 2); } else { ctx.moveTo(-4, -46); ctx.lineTo(4, -46); } ctx.stroke();
  ctx.strokeStyle = body; ctx.lineWidth = 6; ctx.beginPath(); ctx.moveTo(0, -32); ctx.lineTo(0, 28); ctx.stroke();
...
const CanvasEditor = () => {
  ...
  useEffect(() => { const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d'); if (!ctx) return; ctx.clearRect(0, 0, canvas.width, canvas.height); rootIds.forEach((id) => { const object = objects[id]; if (!object || !object.visible) return; const { x, y, rotation, scaleX, scaleY } = object.transform; ctx.save(); ctx.translate(x, y); ctx.rotate((rotation * Math.PI) / 180); ctx.scale(scaleX, scaleY); if (object.type === 'shape') { ctx.fillStyle = object.fill; if (object.shape === 'circle') { ctx.beginPath(); ctx.arc(object.width / 2, object.height / 2, Math.min(object.width, object.height) / 2, 0, Math.PI * 2); ctx.fill(); } else ctx.fillRect(0, 0, object.width, object.height); } else if (object.type === 'text') { ctx.fillStyle = object.fill; ctx.font = `${object.fontSize}px ${object.fontFamily}`; ctx.fillText(object.text, 0, object.fontSize); } else if (object.type === 'character') drawCharacter(ctx, object, assets[object.characterId]); ctx.restore(); }); const selected = selectedIds.length === 1 ? objects[selectedIds[0]] : null; if (selected?.type === 'shape' && selected.visible) { const { x, y, rotation, scaleX, scaleY } = selected.transform; ctx.save(); ctx.translate(x, y); ctx.rotate((rotation * Math.PI) / 180); ctx.scale(scaleX, scaleY); ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.setLineDash([6, 4]); ctx.strokeRect(-2, -2, selected.width + 4, selected.height + 4); ctx.setLineDash([]); ctx.fillStyle = '#fff'; ctx.fillRect(selected.width - HANDLE_SIZE / 2, selected.height - HANDLE_SIZE / 2, HANDLE_SIZE, HANDLE_SIZE); ctx.restore(); } }, [objects, rootIds, selectedIds, assets]);
